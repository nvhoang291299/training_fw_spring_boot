...
public class JdbcRestaurantRepository implements RestaurantRepository {
    ...
    public Restaurant findByMerchantNumber(String merchantNumber) {
        ...
        // throw new RuntimeException("SQL exception occurred finding by
        // merchant number", e);
        // }

        RowMapper<Restaurant> rowMapper = null;
        jdbcTemplate.queryForObject(sql, rowMapper, merchantNumber);
    ...
    /**
     * Advances a ResultSet to the next row and throws an exception if there are
     * no rows.
     * 
     * @param rs
     *            the ResultSet to advance
    ...
    /**
     * Helper method that maps benefit availability policy data in the ResultSet
     * to a fully-configured {@link BenefitAvailabilityPolicy} object. The key
     * column is 'BENEFIT_AVAILABILITY_POLICY', which is a discriminator column
     * containing a string code that identifies the type of policy. Currently
     * supported types are: 'A' for 'always available' and 'N' for 'never
     * available'.
     *
     * More types could be added easily by enhancing this method. For example,
     * 'W' for 'Weekdays only' or 'M' for 'Max Rewards per Month'. Some of these
     * types might require additional database column values to be configured,
     * for example a 'MAX_REWARDS_PER_MONTH' data column.
     * 
     * @param rs
     *            the result set used to map the policy object from database
     *            column values
     * @return the matching benefit availability policy
     * @throws IllegalArgumentException
     *             if the mapping could not be performed
     */

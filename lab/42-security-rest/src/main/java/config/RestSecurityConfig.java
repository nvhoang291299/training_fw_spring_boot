...
public class RestSecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        http.authorizeHttpRequests((authz) -> authz
                ...
                // - Allow DELETE on the /accounts resource (or any
                // sub-resource)
                // for "SUPERADMIN" role only
                // - Allow POST or PUT on the /accounts resource (or any
                // sub-resource)
                ...
                .anyRequest().denyAll()).httpBasic(withDefaults()).csrf(CsrfConfigurer::disable);
        // @formatter:on
    ...
    public InMemoryUserDetailsManager userDetailsService(PasswordEncoder passwordEncoder) {
        ...
        return new InMemoryUserDetailsManager(user, admin,
                superadmin /* Add new users comma-separated here */);
    }
